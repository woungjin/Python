...
# for y in range(3):
#     for i in range(1):
#         print("이름 : %s" %(lst_sec[y][i]))
#         i += 1 
#         print("성별 : %s" %(lst_sec[y][i]))
#         i += 1
#         print("나이 : %s" %(lst_sec[y][i]))
...
# a = []
# for x in range(2,10):  
#     a.append([])                                                                             # a 에 빈 리스트를 멤버로 추가 2단결과를 이 리스트에 추가할려고  
#     for y in range(1,10):
#         a[x-2].append(x*y)                                                                  # a리스트의 인덱스 0번부터 곱셈한 숫자를 추가 하지만 위에 a는 멤버로 싸여잇으니 2단 3단 멤버로 싸여서 곱셈값이 저장됨
# print(a)
# for x in range(2,10) :
#     for y in range(1,10) :
#         print("%s X %s = %s " %(x,y,a[x-2][y-1]) )                                          # 곱셈한 결과는 위에 자리에 있는걸 인덱스 값으로 그대로 가져옴
#     print()
...
while True :
    ...
    print("0. 종료")
    print("-"*30)
    sel = input("메뉴를 선택하세요 :")                                                                   # int로 하면 다른 문자를 입력하면 메뉴창이 그냥 오류가 떠서 안됨 
    if sel =='1':
        print(fr)
        ...
        name = input("변경대상 이름을 입력하세요")
        if name in fr :
            idx = fr.index(name)                                                                        # 삭제할 이름의 인덱스 값을 가져와서 변수에 저장함
        else :
            print("대상 이름이 없습니다")
            os.system('pause')
            continue                                                                                    # 대상 이름이 없다면 뛰어 넘겨서 다시 메뉴로 넘어가게끔
        Mod_name = input("변경할 이름을 입력해 주세요 : ")
        fr[idx] = Mod_name                                                                              # 삭제할대상의 변수 즉 인덱스 번호를 입력한 값 패킹한다
        os.system('pause')
    elif sel =='0':
        print("프로그램 종료")
        break
